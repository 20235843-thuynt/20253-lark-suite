...
let folderToken =
  isInit && folderTokenIdx < args.length && !args[folderTokenIdx].startsWith('--') ? args[folderTokenIdx] : null;
...
if (isInit && !folderToken) {
  ...
  try {
    ...
    try {
      ...
      // Output không phải JSON — sẽ fallback sang regex bên dưới.
    }
    if (!createdToken) {
      const match =
        folderOutput.match(/("token"|"folder_token"|token:)\s*:\s*"([^"]+)"/) ||
        folderOutput.match(/fldcn[a-zA-Z0-9]+/);
...
for (const [key, item] of Object.entries(mapping)) {
  ...
  const relFilePath = item.file; // e.g. "docs/01-prd.md" (tương đối từ gốc repo)
  ...
  if (!item.doc_id) {
    const parentFlag =
      folderToken && !folderToken.startsWith('--') ? `--parent-token ${folderToken}` : `--parent-position my_library`;
    ...
    try {
      ...
      if (!newDocId) {
        const match =
          output.match(/("document_id"|"doc_id"|"token"|document_id:)\s*:\s*"([^"]+)"/) ||
          output.match(/(doccn[a-zA-Z0-9]+|docx[a-zA-Z0-9]+|[a-zA-Z0-9_-]{20,})/);
  ...
  if (item.doc_id) {
    ...
    try {
      ...
      try {
        execSync(`npx lark-cli drive +update-title --token ${item.doc_id} --type docx --title "${docTitle}"`, {
          stdio: 'pipe',
        });
